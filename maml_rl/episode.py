...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._costs_list = [[] for _ in range(batch_size)]
    ...
    @property
    def costs(self):
        """Costs tensor, same shape as rewards."""
        if self._costs is None:
            costs = np.zeros((len(self), self.batch_size), dtype=np.float32)
            for i in range(self.batch_size):
                length = self.lengths[i]
                np.stack(self._costs_list[i], axis=0, out=costs[:length, i])
            self._costs = torch.as_tensor(costs, device=self.device)
            del self._costs_list
        return self._costs
    ...
    def returns(self):
        if self._returns is None:
            ...
            return_ = torch.zeros((self.batch_size,), dtype=torch.float32, device=self.rewards.device)
    ...
    @property
    def cost_advantages(self):
        if self._cost_advantages is None:
            raise ValueError('Cost advantages not computed. Call '
                             '`episodes.compute_cost_advantages(baseline)` first.')
        return self._cost_advantages

    def append(self, observations, actions, rewards, costs, batch_ids):
        for observation, action, reward, cost, batch_id in zip(
                observations, actions, rewards, costs, batch_ids):
            if batch_id is None:
                continue
            ...
            self._costs_list[batch_id].append(cost.astype(np.float32))
    ...
    def compute_cost_advantages(self, cost_baseline, gae_lambda=1.0, normalize=True):
        """Same GAE as compute_advantages but computed on costs."""
        values = cost_baseline(self).detach()
        values = F.pad(values * self.mask, (0, 0, 0, 1))

        deltas = self.costs + self.gamma * values[1:] - values[:-1]
        self._cost_advantages = torch.zeros_like(self.costs)
        gae = torch.zeros((self.batch_size,), dtype=torch.float32, device=self.device)
        for i in range(len(self) - 1, -1, -1):
            gae = gae * self.gamma * gae_lambda + deltas[i]
            self._cost_advantages[i] = gae

        if normalize:
            self._cost_advantages = weighted_normalize(
                self._cost_advantages, lengths=self.lengths)
        return self.cost_advantages
